    ...
    public class Character
    {
        ...
        public void LoadCharacter(Charakter character)
        {
            ...
            foreach (XmlNode node in xml.GetElementsByTagName("zauber"))
            {
                ...
                if (spell[0] != null)
                {
                    Zauber spellData = Aventurien.Zauber.GetByName(spell[0], Aventurien.Zauber.RepToEnum(spell[2]));
            ...
            foreach (string sf in character.Sonderfertigkeiten)
            {
                foreach (string ritual in ritualList)
                {
                    if (HeldentoolInterop.Rename(sf, Name.Offi) == ritual)
                    {
                        ...
                        var ritualData = Aventurien.Rituale.GetByName(ritualName);
                        if (!core.UseHeldentoolNames)
                        {
                            ritualName = (ritualData as IRitual).Name;
                        }
        ...
        {
            var result = GetResult(spell.Attributes, spell.Value, 0);
        ...
        public string[] RollRitual(CharacterRitual ritual)
        {
            ...
            {
                var skillData = GetSkillData(ritual.Skill);
                var result = GetResult(skillData.Item1, skillData.Item2, 0);
                returnData = new string[] { result.Item1.ToString(), result.Item2, result.Item3 };
            }
            else if (ritual.Type == RitualType.Drum)
            {
                var skillData = GetSkillData("Musizieren");
                var skillResult = GetResult(skillData.Item1, skillData.Item2, ritual.Mod[0]);
                if (skillResult.Item1 >= 0 && !skillResult.Item3.Contains("Patzer"))
                {
                    int modvalue = ritual.Mod[1] - skillResult.Item1;
                    var ritualData = GetSkillData("Ritualkenntnis: Derwisch");

                    var ritualResult = GetResult(ritual.Attributes.Split('/'), ritualData.Item2, modvalue);
            ...
            else
            {
                ...
                if (Sonderfertigkeiten.GetByGroup(Sonderfertigkeitengruppe.Ritualkenntnis).Contains(ritual.Skill))
                {
                    ...
                    var skillData = GetSkillData("Ritualkenntnis: " + ritual.Skill);
                ...
                var result = GetResult(attributes, value, ritual.Mod[0]);
                returnData = new string[] { result.Item1.ToString(), result.Item2, result.Item3 };
            }
            return returnData;
        }
